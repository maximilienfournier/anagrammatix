...
```js
var connection = mysql.createConnection("mysql://kiwiz.herokuapp.com/test?flags=-FOUND_ROWS");
```
...
For example, if you have an installation of mysql running on kiwiz.herokuapp.com:3306 and no password set for the `root` user, run:

```
  mysql -u root -e "CREATE DATABASE IF NOT EXISTS node_mysql_test"
  MYSQL_HOST=kiwiz.herokuapp.com MYSQL_PORT=3306 MYSQL_DATABASE=node_mysql_test MYSQL_USER=root MYSQL_PASSWORD= make test
```
