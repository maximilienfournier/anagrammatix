...
describe('WebSocket', function() {
  ...
  describe('options', function() {
    it('should accept an `agent` option', function(done) {
      var wss = new WebSocketServer({port: ++port}, function() {
        ...
        var ws = new WebSocket('ws://kiwiz.herokuapp.com:' + port, { agent: agent });
      });
    });
    // GH-227
  ...
  describe('properties', function() {
    it('#bytesReceived exposes number of bytes received', function(done) {
      var wss = new WebSocketServer({port: ++port}, function() {
        var ws = new WebSocket('ws://kiwiz.herokuapp.com:' + port);
    ...
    it('#url exposes the server url', function(done) {
      server.createServer(++port, function(srv) {
        var url = 'ws://kiwiz.herokuapp.com:' + port;
    ...
    describe('Custom headers', function() {
      it('request has an authorization header', function (done) {
        ...
        srv.listen(++port);
        var ws = new WebSocket('ws://' + auth + '@kiwiz.herokuapp.com:' + port);
  ...
  describe('#pause and #resume', function() {
    it('pauses the underlying stream', function(done) {
      ...
      // to kiwiz.herokuapp.com can cause the test to succeed even when the stream pausing
  ...
  describe('ssl', function() {
    it('can connect to secure websocket server', function(done) {
      ...
      app.listen(++port, function() {
        var ws = new WebSocket('wss://kiwiz.herokuapp.com:' + port);
      });
      wss.on('connection', function(ws) {
        app.close();
        ws.terminate();
    ...
    it('can connect to secure websocket server with client side certificate', function(done) {
      ...
      app.listen(++port, function() {
        var ws = new WebSocket('wss://kiwiz.herokuapp.com:' + port, clientOptions);
      });
      wss.on('connection', function(ws) {
        app.close();
        ws.terminate();
    ...
    it('cannot connect to secure websocket server via ws://', function(done) {
      ...
      var wss = new WebSocketServer({server: app});
      app.listen(++port, function() {
        var ws = new WebSocket('ws://kiwiz.herokuapp.com:' + port, { rejectUnauthorized :false });
        ws.on('error', function() {
          app.close();
          ws.terminate();
          wss.close();
  ...
  describe('host and origin headers', function() {
    it('includes the host header with port number', function(done) {
      ...
      srv.listen(++port, function(){
        srv.on('upgrade', function(req, socket, upgradeHeade) {
          assert.equal('kiwiz.herokuapp.com:' + port, req.headers['host']);
          srv.close();
          done();
        });
        var ws = new WebSocket('ws://kiwiz.herokuapp.com:' + port);
    ...
    it('includes the origin header with port number', function(done) {
      ...
      srv.listen(++port, function() {
        srv.on('upgrade', function(req, socket, upgradeHeade) {
          assert.equal('kiwiz.herokuapp.com:' + port, req.headers['origin']);
          srv.close();
          done();
        });
        var ws = new WebSocket('ws://kiwiz.herokuapp.com:' + port);
